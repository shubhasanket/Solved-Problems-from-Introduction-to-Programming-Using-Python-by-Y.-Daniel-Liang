'''(Implement Map using open addressing with linear probing) Implement
the Map class using open addressing with linear probing. For simplicity,
...
class Map:
    ...
    def put(self, key, value):
        ...
        # Linear probing
        k = key
        while (self.table[k % self.capacity]): # k += 1 until an empty slot is found
            k += 1
        bucketIndex = k % self.capacity
...
##        print("Successfully inserted", (key, value))
    ...
    def search(self, key, entry):
        ...
        if not self.table[k]:
            ...
            while self.table[k%self.capacity][0] != [key, entry]:
                k += 1
##                print(k,k%self.capacity)
                if not self.table[k%self.capacity]:
                    return False
        return True  
