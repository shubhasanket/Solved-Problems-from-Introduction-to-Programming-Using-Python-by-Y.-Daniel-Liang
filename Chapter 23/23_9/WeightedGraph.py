'''23.9 (Finding a minimum spanning tree) Write a program that reads
a connected graph from a file and displays its minimum spanning tree.
The first line in the file contains a number that indicates the number
of vertices (n). The vertices are labeled as 0, 1, ..., n-1. Each subsequent
line describes the edges in the form of u1, v1, w1 | u2, v2, w2 | .... Each
triplet in this form describes an edge and its weight. Figure 23.22 shows
an example of the file for the corresponding graph. Note that we assume the
graph is undirected. If the graph has an edge (u, v), it also has an edge
(v, u). Only one edge is represented in the file. When you construct a graph
both edges need to be considered.

Your program should prompt the user to enter the name of the file, should
read data from a file, create an instance g of WeightedGraph, invoke
g.printWeightedEdges() to display all edges, invoke getMinimumSpanningTree()
to obtain an instance tree of WeightedGraph.MST, invoke tree.getTotalWeight()
to display the weight of the minimum spanning tree, and invoke tree.printTree()
to display the tree.

(Hint: Use new WeightedGraph(list, numberOfVertices) to create a graph, where
list contains a list of WeightedEdge objects. Use new WeightedEdge(u, v, w) to
create an edge. Read the first line to get the number of vertices. Read each
subsequent line into a string s and use s.split("[\\|]") to extract the triplets.
For each triplet, triplet.split("[,]") to extract vertices and weight.)
'''
...
##                vertice = b
    ...
    # Find single source shortest paths 
    def getShortestPath_Adj(self, sourceIndex):
        # T stores the vertices whose path found so far
        T = [sourceIndex]
...
##                vertice = self.matrix[u].index(weight)
    ...
    def getShortestHamiltonianCycle(self):
        al = self.getAllHamiltonianCycles()

        if not al: # al is empty, no hamiltonian cycle found
            return None         # return None as required
        else:
            bestT = MAX_VALUE
            for u in al:
                v = []+u
...
##    print(graph1.getShortestHamiltonianCycle())
    file_n = input("Enter a file name: ")
##    print(vertices)
    file = open(file_n, 'r')
    l = file.readlines()
##    print(l)
    num = eval(l[0].strip())
    print("The number of vertices is",num)
    lst = []
    for i in range (1,len(l)):
        v = l[i].split("|")
        for u in v:
            lst.append([eval(j) for j in u.strip().split(',')])
##    print(lst)
    edges = []
    for u in lst:
        edges.append(u)
        edges.append([u[1],u[0],u[2]])
##      print(edges)
...
##    l = [[] for i in vertices]
##    for u in edges:
##        l[u[0]].append(tuple(u))
##    print("The number of vertices is:", g.getSize())
##    for u in vertices:
##        print("Vertex "+str(u)+":",tuple(l[u]))
##    print('Total weight is', g.
    g.printWeightedEdges()

    t = g.getMinimumSpanningTree()
    print('Total weight is',t.getTotalWeight())
    t.printTree()
