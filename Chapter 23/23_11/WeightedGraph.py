'''*23.11 (Finding shortest paths) Write a program that reads a connected
graph from a file. The graph is stored in a file using the same format
specified in Exercise 23.9. Your program should prompt the user to enter
the name of the file, then two vertices, and should display the shortest
path between the two vertices. For example, for the graph in Figure 23.21, a
shortest path between 0 and 1 may be displayed as 0 2 4 3 1.
'''
...
####            s += str(self.getVertex(i)) + " " + str(i) + "): "
##            for edge in self.queues[i].getLst():
##                if edge.u < edge.v:
##                    s += str(edge.u) + ", " + str(edge.v) + ", "  + str(edge.weight)+" | "
##            print(s)
##            s = s[:-3]
##            s += "\n"
##        return s
...
##    print(graph1.getShortestHamiltonianCycle())
    ...
    vert = input("Enter two vertices (integer indexes): ").split(" ")
    file = open(file_n, 'r')
    l = file.readlines()
##    print(l)
    num = eval(l[0].strip())
...
##    print("A path from", vert[0], "to", vert[1], "is", end = " ")
##    print(eval(vert[0]))
    t = g.getShortestPath(eval(vert[0]))
    t.printPath(eval(vert[1]))
...
s = r'C:\Users\Shubham\AppData\Local\Programs\Python\Python39\Workspace\Chapter_23\23_11\23_9.txt'
