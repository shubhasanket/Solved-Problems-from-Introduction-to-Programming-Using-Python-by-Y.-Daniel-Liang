'''
22.4 (Find connected components) Create a new class named MyGraph
as a subclass of UnweightedGraph that contains a method for finding
all connected components in a graph with the following header:
def getConnectedComponents(): The method returns a list.
Each element in the list is another list that contains all the vertices
in a connected component. For example, for the graph in Figure 22.20b,
getConnectedComponents() returns [[0, 1, 2, 3], [4, 5]].
'''
