'''
22.14 (Hamiltonian cycle) The Hamiltonian path algorithm was implemented
in Supplement VII.C. Add the following getHamiltonianCycle method in the
Graph class:
...
##        print(g1, g2)
    ...
    def getHamiltonianCycle(self): # Using backtracking 
...
##        vertice_0 = vertices[0]
    ...
    def getHamiltonianPath(self): # Using backtracking 
##        vertice_0 = vertices[0]
        for i in range(len(self.vertices)):
            l = [] # list containing the order of the Hamiltonian
                   # cycle
            ...
            while (r!= -1):
                ...
                if DEBUG:
                    print("l =", l)
                    print("r =", r)
                    print("Index of the current vertice we are looking at:",
                          v)
                    print("The index of the next vertice we are looking at:",
                          i)
                if i == (len(self.neighbors[v])-1):
                    # there are no other vertices left in the neighbors
                    # list of v that can be appended
                ...
                if len(l) == len(self.vertices):
                    # this means l is complete
                    # we have  hamiltonian path
                    return l

            # When the graph has no hamiltonian cycle
            return None

    def getAllHamiltonianCycle(self): # Using backtracking
        al = []
...
##    vertices = [4,3,2,1,0]
    edges = [[0,1], [0,4], [1,0], [1,2], [1,4], [2,1], [2,3], [2,4],
             [3,2], [3,4], [4,1], [4,2], [4,3], [4,0]]
##    edges = [[0,1], [0,4], [1,0], [1,2], [2,1], [2,3], [3,2], [3,4], [4,3], [4,0]]
    vertices = [0,1,2,3,4]
...
##    else:
##        print(h)
    h = g.getHamiltonianCycle()
    print(h)
    h = g.getHamiltonianPath()
    print(h)
    h = g.getAllHamiltonianCycle()
    print(h)
    ...
    for _ in range(1000):
        ...
        #Hamiltonian Cycle: 0 1 5 8 6 3 7 4 2 0
        graph = Graph(vertices, edges)
        graph.getHamiltonianCycle()
    t2 = time.time()
    print(t2-t1)
##    print(g.neighbors)
