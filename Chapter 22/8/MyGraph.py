'''
22.8 (Test bipartite) Recall that a graph is bipartite if its vertices can
be divided into two disjoint sets such that no edges exist between vertices
in the same set. Add a new method in Graph to detect whether the graph is
bipartite:
def isBipartite():
'''
